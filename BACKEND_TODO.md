...
- [x] `ResearchSource` 형식 변환 (`adapters.py`, 2026-08-25 완료)
- [x] **검수(품질 평가) 기능 개발** (`reviewer.py`, 2026-08-25 완료) ← 아래 C 참고
- [x] `POST /rag/draft` — 프론트 `NewsletterDraft` 형식 그대로 반환
...
**진행 상황** (2026-08-25 완료)
- [x] `reviewer.py` 신규 작성 — 채점 기준은 `newsletter.py` 것을 그대로 따름
- [x] 프론트 형식(readability / fact_accuracy / coherence)으로 매핑
- [ ] 팀원(moonlight)에게 채점 기준 차용 사실 공유  ← **남음**

실제 검수 결과 예시 (주제: AI 반도체)

```
총점 75 (C 보완 필요)
가독성 90 / 사실정확 80 / 일관성 85
의견: 주장에 대한 근거가 부족한 부분이 있습니다 ...
```

하드코딩된 95점이 아니라 실제로 채점된 값이다.

---

### C-2. 검색 품질 개선 (작업 중 발견해 처리)

검수 점수가 낮아 원인을 보니 **근거 기사부터 엉뚱했다.**
"AI 반도체"를 물었는데 폭염주의보·의료폐기물 기사가 상위로 올라왔다.

| 원인 | 조치 |
|---|---|
| 본문 900자를 통째로 임베딩해 주제가 희석됨 | 검색은 **제목+앞 200자**로, 근거는 본문 전체로 분리 |
| 관련 없어도 k개를 무조건 채움 | 1등보다 크게 뒤처지는 기사는 버림 (`RAG_DISTANCE_MARGIN`) |

**결과**

```
[AI 반도체] AI 칩 기판 품귀 / 반도체 특별회계 / AI 광고 시장 ...
[금리 인상] 한은 금리 딜레마 / 금리 인하 신청 / 금융 AI 비서
[전기차]   기아 EV트렌드 / 현대차 임협 타결 / 성과금
```
