...
**진행 상황** (2026-08-25 완료)
- [x] 백엔드 포트를 8001로 변경 (`.env` 의 `PORT`)
- [x] CORS 허용 추가 (`CORS_ORIGINS` 로 조정 가능)
- [x] 두 서버 동시 기동 확인 — 8000/8001 정상
- [x] CORS 동작 확인 — `localhost:8000` 허용, 그 외 차단
- [ ] 프론트 담당자와 "어느 주소를 부를지" 합의  ← **남음**

프론트엔드가 호출할 주소:

```
http://127.0.0.1:8001/rag/build       뉴스 수집 + 색인
http://127.0.0.1:8001/rag/summarize   요약 (style: brief|newsletter|deep)
http://127.0.0.1:8001/rag/ask         리서치
http://127.0.0.1:8001/rag/news        원본 뉴스 JSON
```
